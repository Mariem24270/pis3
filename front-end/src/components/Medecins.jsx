...
export default function DoctorsPage() {
    ...
    consultations.forEach(c => {
      ...
      if (matchSearch && !doctorsMap.has(c.doctor_name)) {
        doctorsMap.set(c.doctor_name, {
          // ✅ On garde l'ID du doctor (pas de la consultation)
          doctorId:       c.doctor,
          consultationId: c.id,
          name:           c.doctor_name,
          specialite:     c.doctor_specialite,
          photo:          c.doctor_photo || c.photo,
          firstAvailable: new Date(c.date_fin).toLocaleDateString("fr-FR", {
  ...
  // ✅ Navigation vers /consultation avec filtre par médecin
  const handleReserver = (doc) => {
    navigate("/consultation", {
      state: {
        doctorName:     doc.name,
        doctorId:       doc.doctorId,
        specialite:     doc.specialite,
        filterDoctor:   true,   // flag pour activer le filtre côté ConsultationPage
      },
    });
  };
      ...
      <div className="max-w-5xl mx-auto">
        ...
        <header className="mb-16 text-center md:text-left">
          ...
          <p className="text-slate-400 text-sm max-w-lg leading-relaxed font-medium">
            Parcourez notre réseau de santé d'élite. Visualisez les profils et réservez votre créneau en un instant.
          </p>
        </header>
        ...
        {loading ? (
          ...
          <div className="text-center py-20">
            <p className="text-slate-400 font-bold text-lg">Aucun médecin disponible</p>
            <p className="text-slate-300 text-sm mt-2">Modifiez votre recherche ou revenez plus tard.</p>
          </div>
        ) : (
            ...
            {doctorsList.map(doc => (
              ...
              >
                ...
                <div className="flex-1 pr-4">
                  ...
                  {/* ✅ Bouton avec filtre médecin */}
                  <button
                    onClick={() => handleReserver(doc)}
                ...
                <div className="relative">
                  ...
                  <img
                    src={doc.photo
                      ? (doc.photo.startsWith("http") ? doc.photo : `${MEDIA_BASE_URL}${doc.photo}`)
