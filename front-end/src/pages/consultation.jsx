...
import {
  ...
  Clock, X, Users, ShieldCheck, Wallet, ArrowLeft, Search,
} from "lucide-react";
...
function DoctorCard({ doctor, consultations, onBookingSuccess }) {
  ...
  const handleOpenBooking = () => { if (selectedSlot) setIsModalOpen(true); };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (res.ok && (result.ok || result.status === "success")) {
        ...
        alert("❌ " + (result.error || result.message || "Erreur lors de la validation du paiement."));
      }
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40">
          <div className="bg-white w-full max-w-md rounded-2xl shadow-xl overflow-hidden">
            ...
            <div className="p-5">
              <p className="text-sm text-gray-500 mb-4">{new Date(selectedSlot?.date_fin).toLocaleString()}</p>
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <label className={`flex flex-col items-center p-3 border border-dashed rounded-lg cursor-pointer transition ${fileNNI ? "bg-blue-50 border-[#2b6cb0]" : "border-gray-300 hover:bg-blue-50"}`}>
                  ...
                  <label className={`flex flex-col items-center p-3 border border-dashed rounded-lg cursor-pointer transition ${filePaiement ? "bg-blue-50 border-[#2b6cb0]" : "border-gray-300 hover:bg-blue-50"}`}>
                ...
                <div className="bg-gradient-to-r from-[#1a365d]/10 to-[#2b6cb0]/10 p-4 rounded-lg flex items-center gap-3 border border-[#2b6cb0]/20">
                  <div className="bg-gradient-to-r from-[#1a365d] to-[#2b6cb0] p-2 rounded-lg text-white"><Wallet size={18}/></div>
                  <div>
...
export default function Consultation() {
  ...
  // ✅ Récupère les infos passées depuis DoctorsPage
  const { doctorName, filterDoctor } = location.state || {};

  const [consultations, setConsultations] = useState([]);
  const [loading,       setLoading]       = useState(true);
  // ✅ Pré-remplit le filtre si venu depuis DoctorsPage
  const [searchTerm,    setSearchTerm]    = useState(
    filterDoctor && doctorName ? doctorName : ""
  );
  ...
  // ✅ Liste des médecins filtrée par searchTerm
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-[#e6f0fa] to-white py-12 px-4">
      <div className="max-w-5xl mx-auto">
        ...
        {/* ✅ BANDEAU — affiché si venu depuis DoctorsPage avec un médecin précis */}
        {filterDoctor && doctorName && (
          <div className="flex items-center gap-3 bg-white border border-[#2b6cb0]/30 rounded-2xl px-5 py-3 mb-6 shadow-sm">
            <span className="flex h-2 w-2 rounded-full bg-[#2b6cb0] animate-pulse flex-shrink-0"></span>
            ...
            <button
              onClick={() => {
                setSearchTerm("");
                // Efface le state de navigation pour retirer le bandeau
                navigate("/consultation", { replace: true, state: {} });
              }}
          ...
          <div className="space-y-4">
            {doctors.length > 0 ? (
              ...
              <div className="text-center py-16 bg-white rounded-2xl border border-[#2b6cb0]/20">
                <p className="text-[#2b6cb0]/60 text-sm font-medium">Aucun médecin trouvé pour "{searchTerm}"</p>
                <button onClick={() => setSearchTerm("")} className="mt-3 text-xs text-[#2b6cb0] font-bold underline">
